...
static std::vector<std::string> collect_all_formulas() {
    ...
    for (auto &entry : fs::recursive_directory_iterator(base)) {
        ...
        if (ps.find("/Dense1/1M/") == std::string::npos) continue;
...
static const std::string INPUT_FILE = "data/fullsuite/RespondBQR/Dense1/1M/RespondBQR10.jsonl";
...
TEST_CASE("Multi-property anti-redundancy benchmark", "[multi_property]") {
    ...
    SECTION("30 separate monitors") {
        ...
        BENCHMARK_ADVANCED("30 separate monitors")(Catch::Benchmark::Chronometer meter) {
            meter.measure([&] {
    ...
    SECTION("1 combined monitor") {
        ...
        {
            DenseMultiPropertyMonitor count_monitor = createDenseMultiPropertyMonitor(10000);
            ptl_parser parser;
            for (auto &f : formulas) {
                parser.parse_dense(f, count_monitor);
            }
            finalize_monitor(count_monitor);
            std::cout << "[Combined] Total nodes in single monitor: " << count_monitor.nodes.size() << "\n";
        }

        BENCHMARK_ADVANCED("1 combined monitor")(Catch::Benchmark::Chronometer meter) {
            meter.measure([&] {
                // Build fresh monitor each iteration (temporal state is not reusable)
                DenseMultiPropertyMonitor bench_monitor = createDenseMultiPropertyMonitor(10000);
                ptl_parser bench_parser;
                for (auto &f : formulas) {
